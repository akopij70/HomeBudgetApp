...
import sqlite3
import os.path
import dbmanagement.init_db as init_db
...
class Db:
    ...
    def delete(self, table: str, row_specifier=None) -> bool:
        ...
        if table in ["Expense", "Income", "Wallet"]:
            is_elem = self.cursor.execute(
                f"SELECT id FROM {table} WHERE id = {row_specifier}"
            ).fetchone()
            self.cursor.execute(f"DELETE FROM {table} WHERE id = {row_specifier}")

        elif table in ["User", "WishlistItem", "Category"]:
            is_elem = self.cursor.execute(
                f"SELECT name FROM {table} WHERE name = '{row_specifier}'"
            ).fetchone()
            self.cursor.execute(f"DELETE FROM {table} WHERE name = '{row_specifier}'")

        elif table in ["WalletOwnership"]:
            is_elem = self.cursor.execute(
                f"SELECT walletId,userId FROM {table} WHERE walletId = {row_specifier[0]} AND userId = {row_specifier[1]}"
            ).fetchone()
            self.cursor.execute(
                f"DELETE FROM {table} WHERE walletId = {row_specifier[0]} AND userId = {row_specifier[1]}"
            )
    ...
    def set(self, table: str, column: str, row_specifier, new_val) -> None:
        ...
        if table in ["Expense", "Income", "Wallet"]:
            self.cursor.execute(
                f"UPDATE {table} SET {column}='{new_val}' WHERE id = {row_specifier}"
            )
        elif table in ["User", "WishlistItem", "Category"]:
            self.cursor.execute(
                f"UPDATE {table} SET {column}='{new_val}' WHERE name = '{row_specifier}'"
            )
        elif table in ["WalletOwnership"]:
            self.cursor.execute(
                f"UPDATE {table} SET {column}='{new_val}' WHERE walletId = {row_specifier[0]} AND userId = {row_specifier[1]}"
            )
        self.conn.commit()
    ...
    def get(self, table: str, row_specifier=None) -> list:
        ...
        if row_specifier is None:
            ...
            return self.cursor.execute(
                f"SELECT * FROM {table} WHERE id = {row_specifier}"
            ).fetchall()
        elif table in ["User", "WishlistItem", "Category"]:
            return self.cursor.execute(
                f"SELECT * FROM {table} WHERE name = '{row_specifier}'"
            ).fetchall()
        elif table in ["WalletOwnership"]:
            return self.cursor.execute(
                f"SELECT * FROM {table} WHERE walletId = {row_specifier[0]} AND userId = {row_specifier[1]}"
            ).fetchall()
        return []
...
if __name__ == "__main__":  # debug and testing purpose only
    ...
    print(baza_danych.get("Wallet", 10))
